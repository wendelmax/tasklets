...
class PerformanceManager {
  ...
  recordExecutionTime(duration, taskId = null) {
    ...
    // Track per-task history for heuristics
    if (taskId) {
      ...
      if (history.length > 20) { // Keep last 20 for local trend
        history.shift();
      }
    }
  }
  ...
  /**
   * Detects if a task is "inefficient" for Tasklets (too small/fast for the overhead)
   * @param {string} taskId 
   * @param {number} threshold 
   * @param {number} minSamples 
   * @returns {Object|null} Result with average time if inefficient, null otherwise
   */
  getInefficiencyReport(taskId, threshold = 10, minSamples = 5) {
    if (!taskId) return null;
    const history = this.taskExecutionHistory.get(taskId);
    if (!history) {
      // console.log(`[DEBUG] No history for ${taskId}`);
      return null;
    }
    if (history.length < minSamples) {
      // console.log(`[DEBUG] Not enough samples for ${taskId}: ${history.length} < ${minSamples}`);
      return null;
    }

    const avg = history.reduce((a, b) => a + b, 0) / history.length;
    if (avg < threshold) {
      return { avg, samples: history.length };
    }
    return null;
  }
