...
class Tasklets extends EventEmitter {
    constructor(config = {}) {
        ...
        this.allowedModules = config.allowedModules || null; // Optional allowlist

        this.workerPool = [];     // { worker, busy, lastUsed }
        this.idleWorkers = new Set(); // workerObj references for O(1) idle lookup
        this.workerMap = new WeakMap(); // Worker → workerObj O(1) lookup
        this.activeTasks = new Map();
        this.taskQueue = [];
        this.queueHead = 0;       // offset into taskQueue to avoid shift() O(n)
        this.fnCache = new WeakMap(); // function → string cache
    ...
    _fnToString(fn) {
        let cached = this.fnCache.get(fn);
        if (!cached) {
            cached = fn.toString();
            this.fnCache.set(fn, cached);
        }
        return cached;
    }
    ...
    _terminateWorker(workerObj) {
        // First reject any pending tasks on this worker
        this._cleanupWorkerTasks(workerObj.worker, 'Worker was terminated by Tasklets');
        // Actual termination happens inside _cleanupWorkerTasks via pool logic
        // but we ensure the thread is killed here if not already handled
        workerObj.worker.terminate().catch(() => { });
    }

    _getWorker() {
        // 1. O(1) idle worker lookup via Set
        if (this.idleWorkers.size > 0) {
            const workerObj = this.idleWorkers.values().next().value;
            this.idleWorkers.delete(workerObj);
            return workerObj;
        }

        // 2. Check pool capacity with adaptive limits
        const effectiveMax = this.adaptiveManager.getEffectiveMax();
        if (this.workerPool.length >= effectiveMax) {
            return null;
        }

        // 3. Check maxMemory before spawning
        if (this.maxMemory > 0) {
            const freeMem = os.freemem();
            const totalMem = os.totalmem();
            if (((totalMem - freeMem) / totalMem) * 100 > this.maxMemory) {
                this._log('warn', `Memory limit reached. Not spawning new worker.`);
                return null;
            }
        }
        ...
        this.workerMap.set(worker, workerObj);
        return workerObj;
    }
    ...
    _initWorker(worker) {
        worker.on('message', (msg) => {
            ...
            const workerObj = this.workerMap.get(worker);
            if (workerObj) {
                workerObj.busy = false;
                workerObj.lastUsed = Date.now();
                this.idleWorkers.add(workerObj);
                this._processQueue();
            }
        });
    ...
    _cleanupWorkerTasks(worker, errorMessage) {
        // 1. Reject all tasks assigned to this worker
        for (const [taskId, task] of this.activeTasks.entries()) {
        ...
        // 2. Remove worker from pool, map, and idle set
        const workerObj = this.workerMap.get(worker);
        if (workerObj) {
            this.idleWorkers.delete(workerObj);
    ...
    _queueSize() {
        return this.taskQueue.length - this.queueHead;
    }

    _queueShift() {
        if (this._queueSize() === 0) return null;
        const item = this.taskQueue[this.queueHead++];
    ...
    _processQueue() {
        ...
        const entry = this._queueShift();
        if (!entry) return;

        const { taskFn, resolve, reject, args, startTime } = entry;
        ...
        workerObj.worker.postMessage({
            taskId,
            task: typeof taskFn === 'function' ? this._fnToString(taskFn) : taskFn,
            args,
            secret: this.workerSecret
        });
    }
    ...
    configure(config = {}) {
        const workersVal = config.maxWorkers ?? config.workers;
        if (workersVal !== undefined) {
            if (workersVal === 'auto') {
                this.maxWorkers = os.cpus().length;
            } else {
                const val = parseInt(workersVal, 10);
        ...
        if (config.allowedModules !== undefined) this.allowedModules = config.allowedModules;
...
Tasklets.enableAdaptiveMode = defaultPool.enableAdaptiveMode.bind(defaultPool);
Tasklets.setWorkloadType = defaultPool.setWorkloadType.bind(defaultPool);
Tasklets.retry = defaultPool.retry.bind(defaultPool);
