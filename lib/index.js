...
class Tasklets extends EventEmitter {
    ...
    run(taskFn, ...args) {
        ...
        if (typeof taskFn !== 'function' && typeof taskFn !== 'string') return Promise.reject(new Error('Task must be a function'));
    ...
    async runAll(tasks) {
        if (!Array.isArray(tasks)) return Promise.reject(new Error('Tasks must be an array of functions'));
