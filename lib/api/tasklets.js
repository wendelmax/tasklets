...
class Tasklets extends EventEmitter {
        ...
        try {
          if (this.core.isFinished(taskletId)) {
            ...
            if (this.core.hasError(taskletId)) {
              ...
              // Get the actual result from the tasklet
              const result = this.core.getResult(taskletId);
              resolve(result);
            }
          } else {
  ...
  _runAllWithSpawnMany(tasks, options) {
    return new Promise((resolve, reject) => {
      try {
        // Create a factory function that returns the task at the given index
        const factoryFunction = (index) => {
          return tasks[index]();
        };
        
        // Spawn all tasklets efficiently
        const taskletIds = this.spawnMany(tasks.length, factoryFunction);
        
        // Wait for all tasklets to complete using joinMany
        this.joinMany(taskletIds);
        
        // Collect results
        const results = taskletIds.map(taskletId => {
          if (this.hasError(taskletId)) {
            throw new Error(this.getError(taskletId));
          }
          return this.getResult(taskletId);
        });
    ...
    return new Promise((resolve) => {
      ...
      const cleanup = () => {
        try {
          ...
          // Force process exit after a short delay to ensure libuv handles are closed
          setTimeout(() => {
            this.info('Tasklets', 'Forcing process exit to close libuv handles');
            // Force exit with a more aggressive approach
            process.exit(0);
          }, 50);
          ...
          // Force process exit even on error
          setTimeout(() => {
            this.info('Tasklets', 'Forcing process exit after shutdown error');
            process.exit(1);
          }, 50);
