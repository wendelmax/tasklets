 ...
 * that automatically optimizes task execution behavior based on runtime metrics
 * and workload patterns. It shows how the system automatically adjusts worker
 * threads, timeouts, and performance optimization.
 */
...
async function demonstrateAutoScheduler() {
  ...
  console.log('  Auto-scheduling enabled:', tasklets.isAutoSchedulingEnabled());
  ...
  const initialRecommendations = tasklets.getAutoSchedulingRecommendations();
  ...
  const finalRecommendations = tasklets.getAutoSchedulingRecommendations();
...
function analyzePatternDetection(metricsHistory) {
  ...
  Object.entries(patternCounts).forEach(([pattern, count]) => {
    ...
    console.log(`  ${getPatternName(parseInt(pattern))}: ${count} (${percentage}%)`);
  });
}
