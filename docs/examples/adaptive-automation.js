 ...
 * @description This example demonstrates the new automated adaptive configuration system
...
async function demonstrateCPUIntensiveWorkload() {
    ...
    const results = await tasklets.processCPUIntensive(cpuTasks, task => task());
...
async function demonstrateMemoryIntensiveWorkload() {
    ...
    const results = await tasklets.processMemoryIntensive(memoryTasks, task => task());
...
async function demonstrateMixedWorkload() {
    ...
    const results = await tasklets.processBatchAdaptive(mixedTasks, task => task());
